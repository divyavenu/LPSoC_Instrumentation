...
const char* MassStorage::CombineName(const char* directory, const char* fileName)
{
	...
	while (fileName[in] != 0 && fileName[in] != '\n')
	{
		...
		{
			Debug("MS","CombineName() buffer overflow.\n");
      Debug("MS","Name bigger than ");
      Debug(combinedName.Length());
      Debug("\n");
			out = 0;
		}
	}
...
const char* FileStore::CombineName(const char* directory, const char* fileName)
{
	int out = 0;
	int in = 0;
...
bool MassStorage::Delete(const char* directory, const char* fileName)
{
	const char* location = (directory != NULL)
							? CombineName(directory, fileName)
								: fileName;
...
{
	const char* location = CombineName(parentDir, dirName);
...
{
	const char* location = (directory != NULL)
							? CombineName(directory, subDirectory)
								: subDirectory;
...
FileStore::FileStore(void) : combinedName(combinedNameBuff, ARRAY_SIZE(combinedNameBuff))
{
}
